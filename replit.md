...
- **Themed modal**: `client/src/lib/clerkAppearance.ts` factory builds a Clerk `appearance` object driven by `hsl(var(--*))` CSS variables so the sign-in/sign-up modal tracks the active theme (terminal/dark/light) live. `ClerkWithTheme` wrapper inside `ThemeProvider` re-binds the appearance on theme change.
...
- **Pure-CSS auth backdrop** (Task #60, May 02, 2026): the AI-generated JPG backdrops (`backdrop-terminal.jpg`, `backdrop-light.jpg`) — which had off-palette colors, scattered noise glyphs, and a literal "Modal disloga" placeholder artifact — were deleted. They're replaced by a single pure-CSS backdrop in `client/src/index.css` (`.ftf-clerk-backdrop` for the Clerk modal, `.ftf-auth-page-backdrop` for the standalone `/sign-in`, `/sign-up`, `/reset-password` pages). The backdrop is an 8×8 `repeating-linear-gradient` grid keyed off `hsl(var(--primary) / 0.22)` with two ASCII motifs (`> AUTH_READY` top-left, `[FTF / 1.0.4]` bottom-right) rendered via `::before`/`::after`. Because every value derives from `--background` and `--primary`, all three themes are served by a single rule and any future palette change auto-propagates. The standalone pages use `position: fixed inset-0 z-0 pointer-events-none` so the corner motifs always land in the viewport and clicks pass through to the form. The Clerk-modal variant retains the existing `z-index: 100 !important` rule so it sits above the CRT overlay; the standalone-page variant intentionally has no `z-index` declaration so Tailwind's `z-0` keeps the form clickable.
